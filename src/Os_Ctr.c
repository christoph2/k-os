...
void OsCtr_InitCounters(void)
{
    ...
    for (i=(CounterType)0;i<OS_NUM_COUNTERS;++i) {
...
static void OsCtr_UpdateAttachedAlarms(CounterType CounterID)
{
    ...
    for (i=(uint8_least)0;i<__OS_CounterDefs[CounterID].NumAlarms;++i) {
        AlarmID=__OS_CounterDefs[CounterID].AlarmsForCounter[i];
        if (OS_AlarmValue[AlarmID].State==ALM_RUNNING) {
