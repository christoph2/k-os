...
**  CONFIG.
*/
#define OS_MLQ_NUM_PRIORITIES   ((uint8)3)

static TaskType MLQ_QueueData[10];  /*  OS_NUM_TASKS */

const QueueConfigurationType MLQ_QueueDef[OS_MLQ_NUM_PRIORITIES]={ /* MLQ_ReadyQueueDefs */
...
static QueueType MLQ_ReadyQueue[OS_MLQ_NUM_PRIORITIES];
...
void OsMLQ_Init(void)
{
    ...
    for (idx=(uint8)0;idx<OS_MLQ_NUM_PRIORITIES;++idx) {
...
{
    ASSERT((prio!=PRIO_NONE) && (INVERT_NIBBLE(prio)<=OS_MLQ_NUM_PRIORITIES));
...
void OsMLQ_RemoveTask(TaskType TaskID)
{
    ...
    ASSERT((TaskID!=INVALID_TASK) && (TaskID<=OS_NUM_TASKS));
...
{
    ASSERT((new_prio!=PRIO_NONE) && (INVERT_NIBBLE(new_prio)<=OS_MLQ_NUM_PRIORITIES));
