...
void Generator::generate(const OilModel& model, const Options& opts) const {
    ...
    auto output_dir = opts.output_prefix.parent_path();
    auto stem = opts.output_prefix.filename().generic_string();
    auto aliasStem = std::string("Os_Cfg");
    if (!output_dir.empty() && toUpper(stem) != toUpper(aliasStem)) {
        auto copyMaybe = [](const std::filesystem::path& src, const std::filesystem::path& dst) {
            std::error_code ec;
            std::filesystem::create_directories(dst.parent_path(), ec);
            std::filesystem::copy_file(src, dst, std::filesystem::copy_options::overwrite_existing, ec);
        };
        auto make = [&](const std::string& ext) {
            auto p = opts.output_prefix;
            p.replace_extension(ext);
            return p;
        };
        auto makeAlias = [&](const std::string& ext) {
            auto p = output_dir / aliasStem;
            p.replace_extension(ext);
            return p;
        };
        copyMaybe(make(".h"), makeAlias(".h"));
        copyMaybe(make(".c"), makeAlias(".c"));
        copyMaybe(make(".info.txt"), makeAlias(".info.txt"));
        if (opts.generate_orti) {
            copyMaybe(make(".ort"), makeAlias(".ort"));
        }
    }
...
void Generator::writeOsCfgH(const OilModel& model, const Options& opts) const {
    ...
    const ObjectInstance* os = nullptr;
    auto osObjects = filter(model, "OS");
    if (!osObjects.empty()) os = osObjects.front();
    auto osStatus = firstTokenUpper(os ? findAttr(*os, "STATUS") : std::nullopt);
    bool extendedStatus = (osStatus == "EXTENDED");
    auto osBool = [&](const std::string& name, bool fallback) {
        return toBool(os ? findAttr(*os, name) : std::nullopt, fallback);
    };
    bool startupHook = osBool("STARTUPHOOK", true);
    bool shutdownHook = osBool("SHUTDOWNHOOK", true);
    bool pretaskHook = osBool("PRETASKHOOK", true);
    bool posttaskHook = osBool("POSTTASKHOOK", true);
    bool errorHook = osBool("ERRORHOOK", true);
    bool useGetServiceId = osBool("USEGETSERVICEID", true);
    bool useParameterAccess = osBool("USEPARAMETERACCESS", true);
    bool useResScheduler = osBool("USERESSCHEDULER", true);
    bool useO1Scheduler = osBool("O1SCHEDULER", true);
    bool useCallLevelCheck = osBool("USECALLLEVELCHECK", false);
    bool saveStartupContext = osBool("SAVE_STARTUP_CONTEXT", false);
    bool ortiDebug = opts.generate_orti || osBool("ORTI_DEBUG", false);
    ...
    h << "#define OS_AUTOSAR\n";
    h << "#define OS_FEATURE_ORTI_DEBUG          " << (ortiDebug ? "STD_ON" : "STD_OFF") << "\n";
    h << "#define OS_" << (extendedStatus ? "EXTENDED" : "STANDARD") << "_STATUS\n";
    h << "#define OS_USE_STARTUPHOOK             " << (startupHook ? "STD_ON" : "STD_OFF") << "\n";
    h << "#define OS_USE_SHUTDOWNHOOK            " << (shutdownHook ? "STD_ON" : "STD_OFF") << "\n";
    h << "#define OS_USE_PRETASKHOOK             " << (pretaskHook ? "STD_ON" : "STD_OFF") << "\n";
    h << "#define OS_USE_POSTTASKHOOK            " << (posttaskHook ? "STD_ON" : "STD_OFF") << "\n";
    h << "#define OS_USE_ERRORHOOK               " << (errorHook ? "STD_ON" : "STD_OFF") << "\n";
    h << "#define OS_USE_RESSCHEDULER            " << (useResScheduler ? "STD_ON" : "STD_OFF") << "\n";
    h << "#define OS_USE_O1SCHEDULER             " << (useO1Scheduler ? "STD_ON" : "STD_OFF") << "\n";
    h << "#define OS_USE_CALLLEVELCHECK          " << (useCallLevelCheck ? "STD_ON" : "STD_OFF") << "\n";
    h << "#define OS_SCHED_POLICY_MIX\n";
    if (useGetServiceId) h << "#define OS_USE_GETSERVICEID\n";
    if (useParameterAccess) h << "#define OS_USE_PARAMETERACCESS\n";
    if (saveStartupContext) h << "#define OS_FEATURE_SAVE_STARTUP_CONTEXT\n";
...
void Generator::writeOrti(const OilModel& model, const Options& opts) const {
    ...
    const std::string resourceTraceExpr =
        "(Os_ServiceContext.id==OSServiceId_GetResource || Os_ServiceContext.id==OSServiceId_ReleaseResource) && "
        "((uint8)Os_ServiceContext.param1 < OS_NUMBER_OF_RESOURCES) ? "
        "(uint8)Os_ServiceContext.param1 : (uint8)0xFF";
    const std::string comTraceExpr =
        "((Os_ServiceContext.id >= COMServiceId_InitMessage && Os_ServiceContext.id <= COMServiceId_GetMessageStatus) && "
        "((MessageIdentifier)Os_ServiceContext.param1 < COM_NUMBER_OF_MESSAGES)) ? "
        "(MessageIdentifier)(Os_ServiceContext.param1) : (MessageIdentifier)0xFFFF";
    const std::string nmTraceExpr =
        "((Os_ServiceContext.id >= COMServiceId_InitMessage && Os_ServiceContext.id <= COMServiceId_GetMessageStatus) && "
        "((MessageIdentifier)Os_ServiceContext.param1 < COM_NUMBER_OF_MESSAGES) && "
        "(Os_MessageNetworkId[(MessageIdentifier)Os_ServiceContext.param1] >= 0) && "
        "(Os_MessageNetworkId[(MessageIdentifier)Os_ServiceContext.param1] < OS_NUMBER_OF_NETWORKMESSAGES)) ? "
        "(uint16)Os_MessageNetworkId[(MessageIdentifier)Os_ServiceContext.param1] : (uint16)0xFFFF";
    const std::string ipduTraceExpr =
        "((Os_ServiceContext.id >= COMServiceId_InitMessage && Os_ServiceContext.id <= COMServiceId_GetMessageStatus) && "
        "((MessageIdentifier)Os_ServiceContext.param1 < COM_NUMBER_OF_MESSAGES) && "
        "(Os_MessageNetworkId[(MessageIdentifier)Os_ServiceContext.param1] >= 0) && "
        "(Os_MessageNetworkId[(MessageIdentifier)Os_ServiceContext.param1] < OS_NUMBER_OF_NETWORKMESSAGES) && "
        "(Os_NetworkMessages[Os_MessageNetworkId[(MessageIdentifier)Os_ServiceContext.param1]].ipdu_id >= 0) && "
        "(Os_NetworkMessages[Os_MessageNetworkId[(MessageIdentifier)Os_ServiceContext.param1]].ipdu_id < OS_NUMBER_OF_IPDUS)) ? "
        "(uint16)Os_NetworkMessages[Os_MessageNetworkId[(MessageIdentifier)Os_ServiceContext.param1]].ipdu_id : (uint16)0xFFFF";
    ...
    o << "    RUNNINGTASKPRIORITY = \"(OS_TCB[OsCurrentTID].CurrentPriority)\";\n";
    o << "    RUNNINGISR2         = \"(uint8)OsCurrentISRID\";\n";
    o << "    SERVICETRACE        = \"Os_ServiceContext.id\";\n";
    o << "    LASTERROR           = \"OsLastError\";\n";
    o << "    CURRENTAPPMODE      = \"Os_AppMode\";\n";
    if (hasInternalResourceFeature) {
        o << "    INTERNALRESOURCES   = \"BM_InternalResources\";\n";
    }
    if (!resources.empty()) {
        o << "    RESOURCETRACE       = \"" << resourceTraceExpr << "\";\n";
    }
    if (!isrs.empty() || !hwDrivers.empty()) {
        o << "    ISRTRACE            = \"(uint8)OsCurrentISRID\";\n";
    }
    if (!messages.empty()) {
        o << "    COMTRACE            = \"" << comTraceExpr << "\";\n";
    }
    if (!networkMessages.empty()) {
        o << "    NMTRACE             = \"" << nmTraceExpr << "\";\n";
    }
    if (!ipdus.empty()) {
        o << "    IPDUTRACE           = \"" << ipduTraceExpr << "\";\n";
    }
    o << "};\n\n";

    if (!messages.empty()) {
        o << "MESSAGE COM {\n";
        o << "    CURRENTMESSAGE      = \"" << comTraceExpr << "\";\n";
        o << "};\n\n";
    }
    if (!networkMessages.empty()) {
        o << "NETWORKMESSAGE NM {\n";
        o << "    CURRENTNETWORKMESSAGE = \"" << nmTraceExpr << "\";\n";
        o << "};\n\n";
    }
    if (!ipdus.empty()) {
        o << "IPDU IPDUCFG {\n";
        o << "    CURRENTIPDU         = \"" << ipduTraceExpr << "\";\n";
        o << "};\n\n";
    }
    if (hasInternalResourceFeature) {
        size_t intIdx = 0;
        if (hasSchedulerResource) {
            o << "INTERNALRESOURCE Scheduler {\n";
            o << "    STATE    = \"((BM_InternalResources & (1u << " << intIdx << ")) != 0)\";\n";
            o << "    PRIORITY = \"(OS_IntResourceConf[" << intIdx << "].CeilingPriority)\";\n";
            o << "};\n\n";
            ++intIdx;
        }
        for (size_t i = 0; i < internalResources.size(); ++i, ++intIdx) {
            o << "INTERNALRESOURCE " << sanitize(internalResources[i]->name) << " {\n";
            o << "    STATE    = \"((BM_InternalResources & (1u << " << intIdx << ")) != 0)\";\n";
            o << "    PRIORITY = \"(OS_IntResourceConf[" << intIdx << "].CeilingPriority)\";\n";
            o << "};\n\n";
        }
    ...
    for (size_t i = 0; i < orderedTasks.size(); ++i) {
        ...
        o << "    vs_Resource_Count   = \"(OS_TCB[" << (i + 1) << "].ResourceCount)\";\n";
        o << "};\n\n";
    ...
    if (!isrs.empty() || !hwDrivers.empty()) {
        for (size_t ii = 0; ii < isrs.size() + hwDrivers.size(); ++ii) {
            std::string isrName;
            if (ii < isrs.size()) {
                isrName = sanitize(isrs[ii]->name);
            } else {
                isrName = sanitize(hwDrivers[ii - isrs.size()].first + "Timer");
            }
            o << "ISR " << isrName << " {\n";
            o << "    PRIORITY  = \"(Os_Isrs[" << ii << "].priority)\";\n";
            o << "    CATEGORY  = \"(Os_Isrs[" << ii << "].category)\";\n";
            o << "    TYPE      = \"(Os_Isrs[" << ii << "].type)\";\n";
            o << "};\n\n";
        }
    }
    ...
    for (size_t mi = 0; mi < messages.size(); ++mi) {
        auto propAttr = findAttr(*messages[mi], "MESSAGEPROPERTY");
        std::string propStr = propAttr ? *propAttr : "";
        auto propTokenEnd = propStr.find_first_of(" {");
        std::string propToken = propStr.substr(0, propTokenEnd);

        auto filterAttr = findAttr(*messages[mi], "FILTER");
        std::string filterTok = "ALWAYS";
        if (filterAttr && !filterAttr->empty()) {
            auto end = filterAttr->find_first_of(" {");
            filterTok = toUpper(filterAttr->substr(0, end));
        }
        auto mask = parseNumericToken(filterAttr, "MASK").value_or(0);
        auto xVal = parseNumericToken(filterAttr, "X").value_or(0);
        auto minVal = parseNumericToken(filterAttr, "MIN").value_or(0);
        auto maxVal = parseNumericToken(filterAttr, "MAX").value_or(0);
        auto periodVal = parseNumericToken(filterAttr, "PERIOD").value_or(0);
        auto offsetVal = parseNumericToken(filterAttr, "OFFSET").value_or(0);

        auto initAttr = findAttr(*messages[mi], "INITIALVALUE");
        if (!initAttr) initAttr = findAttr(*messages[mi], "MESSAGEPROPERTY");
        auto initialVal = parseNumericToken(initAttr, "INITIALVALUE").value_or(0);

        auto queueAttr = findAttr(*messages[mi], "QUEUESIZE");
        if (!queueAttr) queueAttr = findAttr(*messages[mi], "MESSAGEPROPERTY");
        uint32_t queueSize = parseNumberToken(queueAttr, "QUEUESIZE").value_or(0);

        auto netAttr = findAttr(*messages[mi], "NETWORKMESSAGE");
        if (!netAttr) netAttr = parseFirstToken(findAttr(*messages[mi], "MESSAGEPROPERTY"), "NETWORKMESSAGE");
        auto linkMsg = parseFirstToken(findAttr(*messages[mi], "LINK"), "RECEIVEMESSAGE");
        if (!linkMsg) linkMsg = parseFirstToken(findAttr(*messages[mi], "RECEIVEMESSAGE"), "MESSAGE");

        auto msgSize = messageSize(messages[mi]);
        auto msgType = messageType(messages[mi]);

        o << "MESSAGE " << sanitize(messages[mi]->name) << " {\n";
        o << "    PROPERTY   = \"" << propToken << "\";\n";
        o << "    TYPE       = \"" << msgType << "\";\n";
        o << "    SIZE       = \"" << msgSize << "\";\n";
        o << "    FILTER     = \"" << filterTok << "\";\n";
        o << "    MASK       = \"" << static_cast<unsigned long long>(mask) << "\";\n";
        o << "    X          = \"" << static_cast<unsigned long long>(xVal) << "\";\n";
        o << "    MIN        = \"" << static_cast<unsigned long long>(minVal) << "\";\n";
        o << "    MAX        = \"" << static_cast<unsigned long long>(maxVal) << "\";\n";
        o << "    PERIOD     = \"" << static_cast<unsigned long long>(periodVal) << "\";\n";
        o << "    OFFSET     = \"" << static_cast<unsigned long long>(offsetVal) << "\";\n";
        o << "    INITIAL    = \"" << static_cast<unsigned long long>(initialVal) << "\";\n";
        o << "    QUEUESIZE  = \"" << queueSize << "\";\n";
        o << "    LINK       = \"" << (linkMsg ? *linkMsg : "") << "\";\n";
        o << "    NETWORKMESSAGE = \"" << (netAttr ? *netAttr : "") << "\";\n";
        o << "};\n\n";
    }
...
void Generator::writeTaskIsrStubs(const OilModel& model, const Options& opts) const {
    ...
    s << "void StartupHook(void)\n{\n"
      << "    /* TODO: implement StartupHook */\n"
      << "}\n\n";
    s << "void ShutdownHook(StatusType Error)\n{\n"
      << "    /* TODO: implement ShutdownHook */\n"
      << "    (void)Error;\n"
      << "}\n\n";
    s << "void PreTaskHook(void)\n{\n"
      << "    /* TODO: implement PreTaskHook */\n"
      << "}\n\n";
    s << "void PostTaskHook(void)\n{\n"
      << "    /* TODO: implement PostTaskHook */\n"
      << "}\n\n";
...
void Generator::writeTargetTemplate(const Options& opts) const {
    ...
    std::filesystem::path repo_root_hint = std::filesystem::weakly_canonical(out_dir / ".." / "..");
    cm << "set(_KOS_SRC_DEFAULT \"" << repo_root_hint.generic_string() << "\")\n";
    cm << "if(NOT DEFINED KOS_SRC_PATH)\n";
    cm << "  if(DEFINED ENV{KOS_SRC_PATH})\n";
    cm << "    set(KOS_SRC_PATH \"$ENV{KOS_SRC_PATH}\" CACHE PATH \"k_os source root\")\n";
    cm << "  else()\n";
    cm << "    set(KOS_SRC_PATH \"${_KOS_SRC_DEFAULT}\" CACHE PATH \"k_os source root\")\n";
    cm << "  endif()\n";
    cm << "endif()\n";
    cm << "file(TO_CMAKE_PATH \"${KOS_SRC_PATH}\" KOS_SRC_PATH)\n";
    cm << "if(NOT EXISTS \"${KOS_SRC_PATH}/src/Os_Task.c\")\n";
    cm << "  message(WARNING \"KOS_SRC_PATH='${KOS_SRC_PATH}' missing expected sources; resetting to '${_KOS_SRC_DEFAULT}'\")\n";
    cm << "  set(KOS_SRC_PATH \"${_KOS_SRC_DEFAULT}\" CACHE PATH \"k_os source root\" FORCE)\n";
    cm << "endif()\n\n";
